...
class LRUCache:
    ...
    def set(self, key, value):
        if key in self.hash_map:
            hashmap_value =self.hash_map[key]
            self.node.delete(hashmap_value[1])
            self.node.add_to_head([key, value])
            self.hash_map[key] = [value, self.node.head]
            return
        
        if self.limit is self.size:
            last_hashmap_node = self.node.tail
            self.node.remove_from_tail()
            del self.hash_map[last_hashmap_node.value[0]]
            self.size -= 1
